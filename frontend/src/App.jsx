...
function App() {
    ...
    async function handleDetectIngredientsFromImage() {
        ...
        try {
            ...
            const data = await res.json();
            const ingNames = (data.ingredients || []).map((i) => i.name).filter(Boolean);

            if (ingNames.length === 0) {
                setError("Sanji couldn't confidently detect any ingredients. Try a clearer photo.");
                return;
            }

            // Merge with any existing text and de-duplicate
                    ...
                    {imagePreviewUrl && (
                        <div style={{ marginBottom: "0.5rem" }}>
                            <img
                                ...
                                style={{ maxWidth: "100%", borderRadius: "0.5rem", border: "1px solid #374151" }}
                            />
                            <button
                                type="button"
                                style={{ ...styles.button, marginTop: "0.5rem", padding: "0.4rem 0.8rem" }}
                                onClick={handleDetectIngredientsFromImage}
                                disabled={detecting}
                            >
